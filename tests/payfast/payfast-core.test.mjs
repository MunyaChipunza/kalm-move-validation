...
test("a signed ITN cannot be replayed against a different order", () => {
  const entries = signedItn({ custom_str1: "ord_other" });
  assert.throws(() => validateItnPayload({ entries, order: order(), config: runtime(), headers: {} }), { code: "order_mismatch" });
});
...
test("checkout source offers PayFast only and uses the protected server initiation route", async () => {
  ...
  assert.match(source, /PayFast/);
  assert.doesNotMatch(source, /value="Ozow"/);
  assert.doesNotMatch(source, /value="EFT"/);
