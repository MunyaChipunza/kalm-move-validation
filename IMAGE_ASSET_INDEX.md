# Image Asset Index

## Brand Assets

| File | Use |
| --- | --- |
| `branding/kalm-collective-display-logo.png` | Header, footer and social preview. |
| `branding/ks-active-logo-transparent.png` | KS Active brand card/page. |
| `branding/kalm-move-display-logo.png` | KALM Move brand card/page. |
| `branding/kalm-living-display-logo.png` | KALM Living brand card/page. |
| `branding/kalm-wellness-display-logo.png` | KALM Wellness brand card/page. |
| `branding/kalm-home-display-logo.png` | KALM Home brand card/page. |
| `branding/source-kalm-nyati-transparent.png` | Source buffalo device used for the KALM family logos. |

## Storefront Images

| Folder/File | Use |
| --- | --- |
| `assets/images/home-hero-shop.webp` | Homepage hero image. |
| `assets/images/campaign-hero.webp` | Featured collection banner. |
| `assets/images/ks-active/` | KS Active product cards and product pages. |
| `assets/images/generated/kalm-move/` | KALM Move product cards and product pages. |
| `assets/images/generated/kalm-living/` | KALM Living product cards and product pages. |
| `assets/images/generated/kalm-wellness/` | KALM Wellness product cards and product pages. |
| `assets/images/generated/kalm-home/` | KALM Home product cards and product pages. |
| `assets/images/generated/brand-tiles/` | Brand page hero/tile imagery for new KALM sub-brands. |

Verdict remains unchanged: KALM Move is validation-ready, not bank-ready or production-ready.
