...
const draftPreview = process.env.KALM_THIRD_DRAFT_URL || 'pending_deploy';
...
const thirdScreens = fs.existsSync(screenshotsRoot) ? fs.readdirSync(screenshotsRoot).filter(name => /third-draft|desktop|mobile|catalogue|footer|header|comparison/i.test(name)).sort() : [];
const report = {
