...
const changedFiles = runGit("diff", "--name-only", "origin/master").split(/\r?\n/).filter(Boolean);
...
check("homepage feature carries approved content and model imagery", ["KALM MOVE", "THE SIGNATURE TEE", "An oversized everyday essential in Black and White, finished with KALM’s embroidered signature emblem.", "SHOP THE SIGNATURE TEE", "EXPLORE KALM MOVE", "Adult male model", "Adult female model"].every((value) => script.includes(value)), "approved feature content present");
check("KALM Move page leads with the available tee", script.includes("move-available-now") && script.includes("The first available piece from KALM Move. A premium oversized unisex tee in Black and White, finished with KALM’s signature embroidered emblem.") && script.includes("SHOP NOW"), "available-now section precedes the launching-soon grid");
...
check("KALM Move search terms include tee, colour and audience data", script.includes("(product.colors || []).join(\" \")") && script.includes("product.collection") && tee?.tags?.includes("t-shirts") && tee?.tags?.includes("unisex") && tee?.tags?.includes("new-in"), "search source and tee tags cover requested discovery terms");
...
check("Paystack, courier and checkout implementation are untouched", !changedFiles.some((file) => /(paystack|courier|checkout)/i.test(file)), changedFiles.join(", ") || "no changes");
