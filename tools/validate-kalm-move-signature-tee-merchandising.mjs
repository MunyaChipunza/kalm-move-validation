...
check("KALM Move Men and Women rank the purchasable Tee before Launching Soon products", script.includes('brand === "kalm-move" && ["men", "women"].includes(audience) && sort === "featured"') && script.includes("isKalmMoveAvailableNowProduct(right.product)"), "Men’s and Women’s default featured order prioritises available-now KALM Move product(s)");
check("KALM Move Women displays the available Tee in its approved female White model image", script.includes('audience === "women" && isKalmMoveAvailableNowProduct(product)') && script.includes('getVariantImages(product, color).find((image) => /female-front\\./i.test(image))') && script.includes("displayImage: entry.displayImage"), "Women’s Tee card uses White with the approved female front image");
